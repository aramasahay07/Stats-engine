...
from app.models.specs import QuerySpec, FilterSpec
...
class DuckDBManager:
    ...
    def _apply_filters_sql(self, filters: List[FilterSpec], params: List[Any]) -> str:
        clauses = []
        for f in filters:
            col = duckdb.escape_identifier(f.col)
            op = f.op.lower()
    ...
    def build_query_sql(self, view_name: str, spec: QuerySpec) -> tuple[str, List[Any]]:
        ...
        for c in spec.select:
            select_parts.append(duckdb.escape_identifier(c))
        # measures
        for m in spec.measures:
            select_parts.append(f"{m.expr} AS {duckdb.escape_identifier(m.name)}")
        ...
        for g in (spec.groupby or spec.select):
            groupby_parts.append(duckdb.escape_identifier(g))
        ...
        if spec.order_by:
            order_sql = " ORDER BY " + ", ".join(
                f"{duckdb.escape_identifier(o.col)} {o.dir.upper()}" for o in spec.order_by
            )

        limit_sql = f" LIMIT {int(spec.limit)}"
        offset_sql = f" OFFSET {int(spec.offset)}" if spec.offset else ""

        sql = f"SELECT {', '.join(select_parts)} FROM {view_name}{where_sql}{groupby_sql}{order_sql}{limit_sql}{offset_sql}"
        return sql, params
    ...
    def profile_parquet(self, parquet_path: Path, sample_n: int = 100) -> Dict[str, Any]:
        with self.connect() as con:
            ...
            missing = {}
            for c in cols:
                q = f"SELECT SUM(CASE WHEN {duckdb.escape_identifier(c)} IS NULL THEN 1 ELSE 0 END) FROM ds"
                missing[c] = con.execute(q).fetchone()[0]
            sample = con.execute(f"SELECT * FROM ds LIMIT {int(sample_n)}").fetchdf().to_dict(orient="records")
            return {
                "n_rows": int(n_rows),
                "schema": [{"name": r[0], "type": r[1], "null": r[2], "key": r[3], "default": r[4], "extra": r[5]} for r in schema],
