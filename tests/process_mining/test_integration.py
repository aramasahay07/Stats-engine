from __future__ import annotations

from fastapi.testclient import TestClient

from app.auth.supabase_jwt import get_current_user
from app.main import create_app


def test_process_mining_route_uses_authenticated_user(monkeypatch):
    app = create_app()
    app.dependency_overrides[get_current_user] = lambda: {"user_id": "user-123", "claims": {"sub": "user-123"}}

    called = {}

    async def fake_analyze_process_mining(user_id: str, request):
        called["user_id"] = user_id
        called["dataset_id"] = request.dataset_id
        return {
            "summary": {
                "total_cases": 1,
                "total_events": 2,
                "unique_activities": 2,
                "average_cycle_time": 1.0,
                "median_cycle_time": 1.0,
                "rework_rate": 0.0,
                "variant_count": 1,
                "sla_breach_rate": None,
            },
            "process_map": {"nodes": [], "edges": []},
            "variants": [],
            "bottlenecks": [],
            "rework_loops": [],
            "ai_insights": {
                "executive_summary": "",
                "key_findings": [],
                "recommended_actions": [],
            },
            "target_state": None,
            "toc_analysis": None,
            "cases": [],
            "conformance": None,
            "root_causes": [],
            "initiatives": [],
            "edge_durations": {},
            "expected_path": [],
            "goals": None,
            "cost_inputs": None,
        }

    monkeypatch.setattr(
        "app.routers.process_mining.analyze_process_mining",
        fake_analyze_process_mining,
    )

    client = TestClient(app)
    response = client.post(
        "/process-mining/analyze",
        headers={"x-user-id": "user-123"},
        json={
            "dataset_id": "dataset-1",
            "mapping": {
                "case_id_column": "case_id",
                "activity_column": "activity",
                "timestamp_column": "event_ts",
            }
        },
    )

    assert response.status_code == 200
    assert called == {"user_id": "user-123", "dataset_id": "dataset-1"}
