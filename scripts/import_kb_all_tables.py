...
def insert(table: str, rows: List[Dict[str, Any]], ignore_duplicates: bool = False) -> None:
    if not rows:
        return
...
def main():
    ...
    if resources_rows:
        ...
        for batch in chunked(resources_rows, CHILD_BATCH):
            safe_run("insert stat_resources", lambda b=batch: insert("stat_resources", b, ignore_duplicates=True))
